...
def perform_reformed_search(query_text, docs, doc_vectors, vocab, inverted_index,word_to_index, 
                            top_k=5, select_n=5, n_results=10):

    query_tokens = query_text.lower().split()
    query_vec = compute_query_vector(
        query_tokens,
        vocab,
        word_to_index,      # <-- you forgot this
        inverted_index,
        len(docs)
)
    ...
    correlated = corr_keywords_hybrid(query_text, freq_counter, select_n)
    ...
    rq_vec = compute_query_vector(rq_tokens,vocab,word_to_index,inverted_index,len(docs)
    )
